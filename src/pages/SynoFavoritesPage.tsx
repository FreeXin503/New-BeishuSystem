...
export default function SynoFavoritesPage() {
  ...
  const [queue, setQueue] = useState<Question[]>(() => favorites.map((f) => f.question));
  const [index, setIndex] = useState(0);
  const [question, setQuestion] = useState<Question | null>(() => (favorites[0] ? favorites[0].question : null));
  ...
  const statsText = useMemo(() => "共 " + favorites.length + " 条收藏", [favorites.length]);

  const resetReview = () => {
    setQueue(favorites.map((f) => f.question));
    setIndex(0);
    setQuestion(favorites[0]?.question ?? null);
    setChoice(null);
    setReveal(false);
